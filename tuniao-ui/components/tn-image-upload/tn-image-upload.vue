<template>
  <view v-if="!disabled" class="tn-image-upload-class tn-image-upload">
    ...
    <view v-if="maxCount > lists.length" class="tn-image-upload__add" :class="{'tn-image-upload__add--custom': customBtn}" @tap="selectFile">
      ...
      >
        <view class="tn-image-upload__item-add--icon tn-icon-camera" :style="{color:uploadBtnColor}"></view>
        <view class="tn-image-upload__item-add__tips" :style="{color:uploadTextColor}">{{ uploadText }}</view>
      </view>
      <!-- 自定义添加按钮 -->
      <view>
